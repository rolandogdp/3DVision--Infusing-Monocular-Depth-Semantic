    ...
    transformed_training = depthDataset(csv_file=filename,
                                        transform_rgb_image=transforms.Compose([
                                           ...
                                           # ),
                                            Normalize(mean,
                                                      std)
                                        ]), transform_depth_image=transforms.Compose([Scale(240, Image.NEAREST), RandomHorizontalFlip(), RandomRotate(5), CenterCrop([304,228], [152, 114]), ToTensor()]), transform_segmentation_mask=transform_segmentation_mask)
...
def getTestingData(batch_size=64, csv_filename="images_files.csv"):
    ...
    transformed_testing = depthDataset(csv_file=filename,
                                       transform_rgb_image=transforms.Compose([
                                           ...
                                           Scale(240, Image.NEAREST),
                                           CenterCrop([304, 228], [304, 228]),
                                           ToTensor(is_test=True),
                                       ]),
                                       transform_segmentation_mask=transform_segmentation_mask
                                       )
